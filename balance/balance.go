...
func BasicStats_MaxHP_FromBody(body funcs.Stream) float64 {return funcs.Rou( (math.Pi/(1/body.Cre+1/body.Alt+1/body.Des))*64+16 ,0) }
func BasicStats_Resistance_FromStream(str funcs.Stream) float64 {return funcs.Rou( math.Pi/(1/str.Cre+1/str.Alt+1/str.Des) ,0) }
func BasicStats_MaxPool_FromStream(str funcs.Stream) float64 {return funcs.Rou( math.Sqrt(funcs.Vector(str.Cre,str.Alt,str.Des))*32 ,0) }
func BasicStats_Stream_FromNormaleWithElement(norm float64, element string) funcs.Stream {
  ...
  return funcs.Stream{Cre: funcs.Rou(cre,0), Alt: funcs.Rou(alt,0), Des: funcs.Rou(des,0), Element: element}
}
...
func Regeneration_DotWeight_FromStream(stream funcs.Stream) funcs.Dot {
  ...
  return funcs.Dot{Element:stream.Element,Weight: funcs.Rou( w*(funcs.Rand()*0.5+0.75) ,0) }
}

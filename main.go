...
func PlayerStatus(it Player) {
  ...
  playerTuple = plot.AddRow(row,playerTuple)
  playerTuple = plot.AddRow( fmt.Sprintf("Pool|Max: %0.0f|Current: %d|Rate: %1.0f%%", it.Nature.Pool.Max, len(it.Nature.Pool.Dots), 100*float64(len(it.Nature.Pool.Dots))/float64(it.Nature.Pool.Max) ) ,playerTuple)
  plot.PlotTable(playerTuple, false)
}

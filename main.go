...
func PlayerBorn(mean float64) Player {
  ...
  buffer.Health.Max = mean*mean*100 // from db
  buffer.Health.Current = math.Sqrt(buffer.Health.Max+1)-1 //from db
  current := fmt.Sprintf("Health|Current: %0.1f|Max: %0.1f|Rate: %1.1f%%", buffer.Health.Current, buffer.Health.Max, 100*buffer.Health.Current/buffer.Health.Max)
  ...
  playerTuple = AddRow( fmt.Sprintf("Pool|Current %d|Max: %0.0f|Rate: %1.1f%%", len(buffer.Nature.Pool.Dots), buffer.Nature.Pool.Max, 100*float64(len(buffer.Nature.Pool.Dots))/float64(buffer.Nature.Pool.Max) ) ,playerTuple)
